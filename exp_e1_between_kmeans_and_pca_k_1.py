...
# Parameters
K = 40  # Number of clusters/dictionary elements
n_points = 100
...
# Optimizer
optim = torch.optim.Adam([W1, W2, b], lr=3e-3)
...
for step in range(num_steps):
    # Train the Top-1 SAE (softmax-based)
    ...
    softmax_weights = torch.softmax(code / temperature, dim=1)
    reconstruction = torch.mm(softmax_weights, W2.T / W2_norm.unsqueeze(1))
    ...
    if step % 100 == 0:
        print(f"Step {step}/{num_steps}: SAE Loss = {loss.item():.6f}, K-means MSE = {kmeans_mse:.6f}, PCA MSE = {pca_mse:.6f}")
    ...
    if step % visualization_interval == 0:
        ...
        with torch.no_grad():
            ...
            partition_indices = torch.topk(Z, dim=1, k=1)[1]
            ...
            # Compute top-1 indices for each data point (for usage visualization)
            indices = torch.topk((X @ W1.T / W1_norm) + b, dim=1, k=1)[1].squeeze(1)
            dict_usage = torch.zeros(K)
            for idx in indices:
                dict_usage[idx] += 1
            ...
            '''
            for i in range(n_points):
                w = W1[indices[i]].detach().cpu().numpy()
