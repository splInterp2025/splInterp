...
for step in range(num_steps):
    # Train the Top-k SAE with hard top-k selection
    W1_norm = W1.square().sum(1).sqrt()
    W2_norm = W2.square().sum(0).sqrt()
    
    code = (X @ W1.T / W1_norm) + b
    
    # Explicit top-k selection with hard weights
    topk_values, topk_indices = torch.topk(code, k=k_active, dim=1)
    
    # Create sparse weights using only top-k elements with equal weighting (hard selection)
    sparse_weights = torch.zeros_like(code)
    for i in range(X.shape[0]):
        # Assign equal weights (1/k_active) to the top-k elements
        sparse_weights[i, topk_indices[i]] = 1.0 / k_active
    ...
    if step % visualization_interval == 0:
        ...
        with torch.no_grad():
            ...
            Z = (grid_points @ W1.T / W1_norm) + b            # For visualization, compute top-k for grid points too
            grid_topk_values, grid_topk_indices = torch.topk(Z, k=k_active, dim=1)
            grid_sparse_weights = torch.zeros_like(Z)
            for i in range(grid_points.shape[0]):
                # Hard equal weighting for top-k
                grid_sparse_weights[i, grid_topk_indices[i]] = 1.0 / k_active
            ...
            for i in range(n_points):
                ...
                for j, idx in enumerate(topk_indices[i]):
                    ...
                    axes[1].plot([X_np[i, 0], w[0]], [X_np[i, 1], w[1]], 
                                'k-', alpha=line_alpha)
            '''
            axes[1].set_title(f"Top-{k_active} SAE (Hard Selection, Loss: {loss.item():.6f})", fontsize=16)
            axes[1].set_xlim(-2, 2)
            axes[1].set_ylim(-2, 2)
        ...
        plt.suptitle(f"Step {step}: K-means vs. Top-{k_active} SAE vs. PCA Extension", fontsize=28)
        plt.tight_layout()
        plt.pause(0.01)
